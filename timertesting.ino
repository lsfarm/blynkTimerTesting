...
#1 Never start another timer in the loop if it is already running if you do, you'll create 2 timers and the first timers timerID is easily lost. (Both timers will run and you'll never be able to dig up the first timer and kill it)
    UPDATE: this does seem to work - if (!timer.isEnabled(loopT5)) {loopT5 = timer.setInterval(1000L, LEDToggle);}
        I was hoping  >>if (!timer.isEnabled(loopT5)) {loopT5 = timer.setInterval(1000L, LEDToggle);}<< would fix this but for some reason all timers (??1-16??)are enabled at setup() << they aren't this was before I learned the int myTimer = 99; trick
#2 When using setTimer and setTimeout (the ones that will expire) in the loop be sure to set their ID back to ??-1?? ??99?? once finished with them if you don't that expired timerID will come back to haunt you
    timer.toggle(0) seems to toggle a lot of timers.. all the unused ones <<UPDATE on this -- use int myTimer = 99; !!
    it's quit easy to end end with a lot of timers with the same ID's or better said pointers
#3 If you try to delete a timer at the top of the loop using its ID you'll delete the first timer created in setup because calling timer.delete(ID4) before timer.Setup(ID4)
    makes ID4 in timer.delete equal to timer slot 0
*/
...
BLYNK_WRITE(V1) {//this method allows loop4() to be run mutiple times (If you press V1 5 times loop4() will be ran 5 times) each in another timer slot
    button1 = param.asInt();
    if (button1) {
        loopT3 = timer.setTimeout(30000L, [] () { loop4(); loopT3 = 99;  } );
        timer.deleteTimer(loopT4);
        loopT4 = 99;
    }
}
BLYNK_WRITE(V2) {//if timer is already running, don't start it again --This helps, but doesn't entirely fix the problem.  actually mabye it does now that I'm doing loopT4 = 99; above setup()
    button2 = param.asInt();
    if (button2) {
            if (!timer.isEnabled(loopT4)) {// to more I study this the more I think an if like this is all thats needed
                loopT4 = timer.setInterval(5000L, LEDToggle);
            }
    }
}
BLYNK_WRITE(V3) {
    ...
    if (button3) {
            if (timer.isEnabled(loopT4)) {// to more I study this the more I think an if like this is all thats needed
                    timer.restartTimer(loopT4);
                    terminal.println(Time.format("%r - T4 timerReset"));
                    terminal.flush();
            }
...
BLYNK_WRITE(V4) {//this seems to be the fix I've been looking for--hoping to take this idea back into my other code files. 1/17/21-Testing this
    button4 = param.asInt(); 
    if (button4) { //1/18/21 -- is the check for 99 in the next line down nessicary?
